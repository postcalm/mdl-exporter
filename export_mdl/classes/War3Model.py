...
class War3Model:
    ...
    def get_parent(obj):
        ...
        if parent.type == "EMPTY" and (
                parent.name.startswith("Bone_") or parent.name.startswith("bone_") or parent.name.startswith(
            "helper_")):
            return parent.name
    ...
    def from_scene(self, context, settings, report):
        ...
        for obj in objs:
            ...
            if hasattr(obj, "mdl_billboard"):
                ...
                billboard_lock = (bb.billboard_lock_z, bb.billboard_lock_y,
                                  bb.billboard_lock_x)  # NOTE: Axes are listed backwards (same as with colors)
            ...
            # Collision Shapes
                ...
                if "Box" in obj.name:
                    ...
                    for corner in (
                        ...
                        corners.append(
                            mat.to_quaternion()
                            @ Vector(
                                abs(x * obj.empty_display_size * settings.global_matrix.median_scale) * y
                ...
                if (obj.name.startswith("SND") or
                        obj.name.startswith("UBR") or
                        obj.name.startswith("FTP") or
                        obj.name.startswith("SPL")):
        ...
        for tag in tags:
            for obj in self.objects[tag]:
                self.object_indices[obj.name] = index
                self.objects_all.append(obj)
                vertices_all.append(obj.pivot)
        ...
        particles = list(self.objects["particle"]) + list(self.objects["particle2"]) + list(
            self.objects["ribbon"])
        ...
        self.global_extents_min, self.global_extents_max = calc_extents(vertices_all) if len(vertices_all) else (
            (0, 0, 0), (0, 0, 0))
    ...
    def to_scene(self, context, global_matrix, folder):
            ...
            def create_node(index, offset):
                ...
                if texture.is_replaceable:
                    ...
                    if mdl_layer.replaceable_id < 3:
                        ...
                        color_node.outputs[0].default_value = (1.0, 0.0, 0.0, 1.0)  # Team color
                    ...
                    if image_name in bpy.data.images:
                        ...
                        print("Loading image:", import_path)
                        if os.path.exists(import_path):
                            img = bpy.data.images.load(import_path)
                            node.image = img
                        else:
                            print(f"Image at path {import_path} does not exist!")
        ...
        def animate_bone(node):
            ...
            if node.anim_loc is not None:
                ...
                node.anim_loc.to_fcurves(
                    ...
                    armature_obj,
                    "location",
                    f'pose.bones["{node.name}"].location',
                    matrix
                )
            if node.anim_rot is not None:
                ...
                node.anim_rot.to_fcurves(
                    ...
                    armature_obj,
                    "rotation_quaternion",
                    f'pose.bones["{node.name}"].rotation_quaternion'
                )
        ...
        if len(skinned_bone_ids):
            ...
            # Create animations
            armature_bones = [node_map[id_] for id_ in node_map if node_map[id_].name in armature.bones]
        ...
        for geoset_id, geoset in enumerate(self.geosets):
            ...
            faces = [tuple(geoset.triangles[i:i + 3])
                     for i in range(0, len(geoset.triangles), 3)]  # Group triangles into tuples of 3
            ...
            if is_skinned:
                ...
                mesh_obj = bpy.data.objects.new(f"Geoset {geoset_id}", mesh)
        ...
        for node_type in self.objects:
            for node in self.objects[node_type]:
                ...
                nodes = ("helper", "bone", "attachment", "eventobject", "collisionshape")
                if node_type in nodes:
                    node_name = node.name

                    if node_type == "helper" and not node_name.startswith("Bone_"):
                        node_name = f"Bone_{node_name}"
                    ...
                    print("Camera FOV:", node.field_of_view)
                    ...
                    if node.emission_rate_anim is not None:
                        node.emission_rate_anim.to_fcurves(psys, settings, "emission_rate",
...
if __name__ == "__main__":
    ...
    model = War3Model(bpy_context)
    fake_operator = type("Op", (), {"report": lambda _: print(*_)})
    operator = fake_operator
    model.from_scene(bpy_context, settings, operator.report)
